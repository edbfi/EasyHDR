...
All PRs, default-branch pushes and manual dispatches run the same required gate.
`ci / required` requires the native Windows build, tests and GUI startup smoke,
security workflow, portable Miri workflow and repository hygiene. Missing, skipped,
cancelled or failed required jobs block merging.

Shared actions, workflows and presets use immutable `v4.0.0` references. Renovate
is the sole dependency merger: the shared `automerge.json` preset arms GitHub
auto-merge with rebase merges, and GitHub merges only once every required CI and
policy check passes on the current head. Shared Renovate policy updates remain
manual; release-age rules, holds and repository-specific updater ownership still
apply. The legacy Actions merger and its comment commands are retired.
...
emitted policy context alongside all existing application/content checks, pinned to
GitHub Actions, with strict up-to-date branch protection. Preserve stronger review
requirements. Explicit CI dispatches do not substitute for a missing metadata
policy result. Review exact head/base, full diffs and all required results before a
bootstrap merge, then verify resulting default-branch CI. Repository-specific
updater ownership and manual publication or delivery controls remain unchanged.
...
a pinned nightly; its existing symbolic-alignment/isolation flags are retained.
Broader best-effort Miri runs stay weekly because their dependency/FFI limitations
are already documented in the workflow. Renovate maintains the nightly pin through
PRs that must pass the portable Miri test before merging.
...
current through PRs. Eligible dependency updates use the checked Renovate merge
policy described above; shared Renovate configuration changes remain manual.
